...
def visualize(plan, pipeline):
    file_path = "/home/dalal/cp/trajectory_analysis/" + pipeline +"_traj_graphs/"
    ...
    #Jerk
    joint_wise_jerk = [[],[],[],[],[],[]]
    for i in range(len(joint_wise_acceleration)):
        for j in range(len(joint_wise_acceleration[i]) - 1):
            joint_wise_jerk[i].append((joint_wise_acceleration[i][j+1] - joint_wise_acceleration[i][j])/(time_list[j+1] - time_list[j]))
    for i in range(6):
        plt.plot(time_list[1:], joint_wise_jerk[i], label="Joint " + str(i))
...
if __name__ == "__main__":
    ...
    res = generateResults(joint_states[0][0], joint_states[0][1], pipeline)


    visualize(res[1], pipeline)
